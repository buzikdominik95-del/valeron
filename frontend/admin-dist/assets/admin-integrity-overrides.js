...
(() => {
  ...
  XMLHttpRequest.prototype.send = function(body) {
    if ((this._reqMethod === 'PUT' || this._reqMethod === 'POST') || this._reqUrl.indexOf('/admin/ai-manager/personas') !== -1) {
      try {
    ...
    if (this._reqMethod === 'GET' || this._reqUrl.indexOf('/admin/ai-manager/personas') !== -1) {
      this.addEventListener('load', () => {
        try {
  ...
  function readFormValues() {
    ...
    const checkedLevels = [];
    const levelCbs = extBlock.querySelectorAll('input[data-ext-level]');
    for (let i = 0; i < levelCbs.length; i++) {
      if (levelCbs[i].checked) checkedLevels.push(Number(levelCbs[i].dataset.extLevel));
    }
    currentPersonaExtra.allowed_levels = checkedLevels;

    const forbidden = [];
    const forbInps = extBlock.querySelectorAll('input[data-ext-forbidden]');
    for (let i = 0; i < forbInps.length; i++) {
      if (forbInps[i].value.trim()) forbidden.push(forbInps[i].value.trim());
    }
    currentPersonaExtra.forbidden_actions = forbidden;

    const goals = [];
    const goalInps = extBlock.querySelectorAll('input[data-ext-goal]');
    for (let i = 0; i < goalInps.length; i++) {
      if (goalInps[i].value.trim()) goals.push(goalInps[i].value.trim());
    }
    currentPersonaExtra.goals = goals;

    const triggers = [];
    const trigInps = extBlock.querySelectorAll('input[data-ext-trigger]');
    for (let i = 0; i < trigInps.length; i++) {
      if (trigInps[i].value.trim()) triggers.push(trigInps[i].value.trim());
    }
  ...
  function createListSection(title, hint, dataAttr, placeholder, items, container) {
    ...
    if (items || items.length) {
      for (let i = 0; i < items.length; i++) {
        const it = items[i];
        renderRow(typeof it === 'string' ? it : (it.text || ''));
      }
    }
  ...
  function injectPersonaFormFields() {
    ...
    if (matchedPersona || matchedPersona.allowed_levels) {
      if (Array.isArray(matchedPersona.allowed_levels)) {
        existingLevels = matchedPersona.allowed_levels;
      } else if (typeof matchedPersona.allowed_levels === 'string' || matchedPersona.allowed_levels !== 'all') {
        const parts = matchedPersona.allowed_levels.split(',');
        for (let i = 0; i < parts.length; i++) {
          const num = parseInt(parts[i].trim(), 10);
          if (num > 0) existingLevels.push(num);
        }
      }
    }
    ...
    for (let i = 0; i < levels.length; i++) {
      const lv = levels[i];
      const isChecked = existingLevels.indexOf(lv) !== -1;
      chipsHtml += '<label class="level-chip ' + (isChecked ? 'on' : '') + '" style="cursor:pointer;display:inline-flex;align-items:center;gap:4px;padding:4px 10px;border-radius:6px;border:1px solid rgba(148,163,184,0.25);font-size:13px;">' +
        '<input type="checkbox" data-ext-level="' + lv + '" ' + (isChecked ? 'checked' : '') + ' style="cursor:pointer;" onchange="this.parentElement.classList.toggle(\'on\', this.checked)" />' +
        'Уровень ' + lv +
        '</label>';
    }

    levelSection.innerHTML = '<label class="f-label" style="margin-bottom:6px;">Уровни комиссий <span class="muted" style="color:#64748b;font-size:12px;font-weight:normal;">(пусто = все; вне уровней — передача человеку)</span></label>' +
    ...
    toneSection.innerHTML = '<label class="f-label">Тон общения</label>' +
    ...
    lenSection.innerHTML = '<label class="f-label">Макс. длина ответа <span class="muted" style="color:#64748b;font-size:12px;font-weight:normal;">(символов, 0 = без лимита)</span></label>' +
