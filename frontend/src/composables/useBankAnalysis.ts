 ...
 * успевал ни за одной.
 *
 * ОТСЮДА И ДЛИТЕЛЬНОСТЬ. Тринадцать банков по 2.5 с дали бы 32 секунды
 * ожидания — столько никто не ждёт. Последовательный проход требует уложить
 * одну проверку примерно в секунду: 0.9–1.4 с на банк дают около 15 секунд
 * на весь список. Это заметно дольше прежних 600 мс (строку успеваешь
 * прочитать) и втрое короче того, во что превратился бы список при 2.5 с.
 *
 * Разброс, а не одно число: одинаковые интервалы подряд слышны как метроном,
 * и экран выглядит нарисованным заранее, а не отвечающим на запрос.
 */
const CHECK_MIN_MS = 900
const CHECK_MAX_MS = 1400
