 ...
 * и экран выглядит нарисованным заранее, а не отвечающим на запрос. Ширина
 * разброса оставлена прежней по доле — ±6% вокруг семи секунд.
 *
 * ПОЛОСА В СТРОКЕ ЖИВЁТ СТОЛЬКО ЖЕ. Длительность заливки в VelBankRow берётся
 * из переменной --vel-bank-load-ms, и значение ей ставит этот же модуль (см.
 * CHECK_AVG_MS ниже). Разойдись они — полоса добегала бы до края за полторы
 * секунды и семь секунд стояла бы полной у «проверяющегося» банка.
 */
const CHECK_MIN_MS = 6600
const CHECK_MAX_MS = 7400
