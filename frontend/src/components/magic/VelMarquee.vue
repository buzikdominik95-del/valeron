...
<style scoped>
/*
  Колонка с относительным корнем: кнопка паузы позиционируется от него.

  Кнопка свёрнута в точку и разворачивается только под клавиатурным фокусом
  (разбор — в шапке VelMarqueePause), то есть в обычном состоянии она из
  потока выключена и своей строки не занимает: gap между ней и лентой не
  возникает, потому что абсолютный элемент flex-элементом не считается.

  position: relative здесь обязателен. Без него кнопка отсчитывалась бы от
  ближайшего позиционированного предка выше по дереву и в развёрнутом виде
  уезжала бы в произвольное место секции.
*/
.vel-marquee {
